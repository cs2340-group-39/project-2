...
@api.post("create-wrapped-data", response={201: WrappedResponseSchema}, auth=SpotifyLinkedTokenAuthenticator())
def create_wrapped_data(request: HttpRequest):
    user = request.auth

    try:
        artist_response = request_spotify(user=user, query="artists", time_range="medium_term", limit=3, offset=random.randint(0, 10))
    except Exception as e:
        print(e)

    try:
        track_response = request_spotify(user=user, query="tracks", time_range="medium_term", limit=3, offset=random.randint(0, 10))
    except Exception as e:
        print(e)
    ...
    for track in track_response["items"]:
        ...
        track_object = TrackSchema(track_name=track_name, track_cover_url=track_cover_url, artist_name=artist_name, track_type=track_type)
    ...
    return 201, WrappedResponseSchema(id=new.id, username=user.username, artists=artists, tracks=tracks)
...
def get_wrapped_data_for_current_user(request: HttpRequest):
    ...
    return 200, {"items": [WrappedResponseSchema(id=item.id, username=user.username, artists=item.artists, tracks=item.tracks).model_dump() for item in wrapped_objects]}
...
def get_public_wrapped_data(request: HttpRequest):
    ...
    return 200, {"items": [WrappedResponseSchema(id=item.id, username=item.profile.user.username, artists=item.artists, tracks=item.tracks).model_dump() for item in wrapped_objects]}
