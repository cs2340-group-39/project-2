...
CREATE
OR REPLACE FUNCTION fn_create_db_user_on_supabase_user_create () RETURNS TRIGGER SECURITY DEFINER AS $$
BEGIN
  INSERT INTO private.users_supabaseuser (
    uuid,
    email,
    password,
    username,
...
CREATE
OR REPLACE FUNCTION fn_delete_db_user_on_supabase_user_delete () RETURNS TRIGGER SECURITY DEFINER AS $$
BEGIN
...
CREATE
OR REPLACE FUNCTION fn_update_db_user_on_supabase_user_update () RETURNS TRIGGER SECURITY DEFINER AS $$
BEGIN
  -- Only update if email or password has changed
  IF (NEW.email <> OLD.email OR NEW.encrypted_password <> OLD.encrypted_password) THEN
    UPDATE private.users_supabaseuser
    SET
      email = NEW.email,
      password = CASE
        WHEN NEW.encrypted_password <> OLD.encrypted_password THEN concat('bcrypt$', NEW.encrypted_password)
        ELSE password
      END,
...
CREATE
OR REPLACE FUNCTION fn_set_db_user_active_on_supabase_user_email_confirm () RETURNS TRIGGER SECURITY DEFINER AS $$
BEGIN
...
CREATE TRIGGER tg_on_supabase_user_delete BEFORE DELETE ON auth.users FOR EACH ROW
