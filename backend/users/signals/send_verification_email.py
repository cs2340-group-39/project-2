...
def send_verification_email(sender, instance, created, **kwargs):
    if not (instance.is_staff or instance.is_superuser) and (created and not instance.is_active):
        code = uuid.uuid4()

        User.objects.filter(pk=instance.uuid).update(email_verification_code=code, email_verification_code_created_at=timezone.now())
