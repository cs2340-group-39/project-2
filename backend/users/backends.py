...
class TokenAuthenticationBackend(BaseBackend):
    def authenticate(self, request: HttpRequest, token=None):
        ...
        try:
            ...
            try:
                jwt.decode(token, get_token_secret_key(user), algorithms=["HS256"], options={"verify_signature": True})
            except (InvalidTokenError, ExpiredSignatureError, InvalidSignatureError, InvalidAlgorithmError):
                return None
...
class SpotifyLinkedAuthenticationBackend(TokenAuthenticationBackend):
    def authenticate(self, request: HttpRequest, token_of_spotify_linked_user=None):
        user = super().authenticate(request, token_of_spotify_linked_user)

        if user:
            profile = user.profile_for_user
            if profile.spotify_access_token and profile.spotify_refresh_token and profile.spotify_access_token_expires_at:
                if (profile.spotify_access_token_expires_at - timezone.now()) < timedelta(minutes=10):
                    try:
                        credentials = base64.b64encode(f"{settings.SPOTIFY_CLIENT_ID}:{settings.SPOTIFY_CLIENT_SECRET}".encode()).decode()

                        response = requests.post(
                            "https://accounts.spotify.com/api/token", headers={"Content-Type": "application/x-www-form-urlencoded", "Authorization": f"Basic {credentials}"}, data={"grant_type": "refresh_token", "refresh_token": profile.spotify_refresh_token, "client_id": settings.SPOTIFY_CLIENT_ID}
                        )
                        ...
                        profile.spotify_access_token_expires_at = timezone.now() + timedelta(seconds=data["expires_in"])
                        profile.save()
