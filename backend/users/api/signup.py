...
def signup(request: HttpRequest, data: UserRequestSchema):
    try:
        if User.objects.filter(email=data.email).exists():
            return 400, UserErrorResponseSchema(detail="User with this email already exists.")
        ...
        return 201, UserResponseSchema(uuid=str(user.uuid), username=user.username, email=user.email)

    except ValidationError as e:
        return 400, UserErrorResponseSchema(detail=str(e))
    except Exception:
        return 400, UserErrorResponseSchema(detail="Failed to create user. Please try again.")
