...
@api.post("login", response={200: UserResponseSchema, 400: UserErrorResponseSchema})
def login(request: HttpRequest, data: UserRequestSchema):
    try:
        try:
            user = User.objects.get(email=data.email)
        except User.DoesNotExist:
            return 400, UserErrorResponseSchema(detail="Invalid email or password.")
        ...
        return 400, UserErrorResponseSchema(detail="Failed to login user. Please try again.")
