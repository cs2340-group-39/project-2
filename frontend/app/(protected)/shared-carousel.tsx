...
export function SharedCarousel({ wrappedData }: { wrappedData: WrappedData }) {
    ...
    for (let i = 0; i < wrappedData.tracks.length; i++) {
        ...
        const newArtistSlide: CarouselItem = {
            title: `Track #${i + 1}`,
            items: [track.track_name, track.artist_name, track.track_cover_url],
        };
                            ...
                            <Card className="w-full h-full">
                                <CardContent className="flex aspect-square items-center justify-center p-6">
                                    <div className="flex flex-col">
                                        <h3 className="text-lg sm:text-xl font-bold text-center">{section.title}</h3>
                                        <div className="text-center">
                                            {section.items.map((item, itemIndex) => (
                                                <div key={`item-${sectionIndex}-${itemIndex}`}>{itemIndex === 2 ? <img className="p-4" src={item} alt={`${section.title} image`} /> : <p className="text-pretty sm:text-lg">{item}</p>}</div>
                                            ))}
                                        </div>
                                    </div>
                                </CardContent>
