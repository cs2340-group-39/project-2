...
export async function GET(request: NextRequest) {
    ...
    const accessTokenVerificationResponse = await fetch("http://backend:8000/private/users/api/verify-access-token", {
        method: "POST",
        headers: {
    ...
    if (!accessTokenVerificationResponse.ok) {
        console.log(`Unexpected error verifying access token: ${accessTokenVerificationResponse.statusText}`);
    }
    ...
    return NextResponse.redirect(new URL("/users/api/logout", process.env.NEXT_PUBLIC_BASE_URL));
}
